...
def getstate(url):
    ...
    for d in dateSet:
        ...
        if ( dead == None ):
            ...
            D = dead[d]
        all.append({'T': d, 'A': A, 'I': I, 'D': D, 'H': None})
    all.sort(key=lambda s: s['T'], reverse=True)
    return all

statesData = []
urlUSA = "https://www.worldometers.info/coronavirus/"
...
tab = soup.find('table', id="main_table_countries_today")
telo = tab.find('tbody')
rows = telo.find_all('tr')
for row in rows:
    ...
    pop = cols[14].text.strip()
    subj['population']  = int(pop.replace(',',''))
    tst = cols[12].text.strip()
    if ( len(tst) >0 ):
        subj['tests']   = int(tst.replace(',',''))
    else:
        subj['tests']   = None
...
for su in statesData:
    regda = []
    ref = "https://www.worldometers.info/coronavirus/" + su["ref2"]
    print(ref)
    sta = getstate(ref)
...
#    write_file.write(json_string)
with open('DATAS/lastEngWorld.json', "w") as write_file:
    write_file.write(json_string)
